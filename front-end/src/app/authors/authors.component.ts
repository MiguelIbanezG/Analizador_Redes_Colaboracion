...
export class AuthorsComponent {
  ...
  normalized() {
    ...
    for (const author of this.completeAuthor) {
      ...
      if (!processedAuthors.includes(normalizedAuthor)) {
        ...
        const index = uniqueAuthors.findIndex(
          (a) =>
        ...
        next: (response: string[]) => {
          if (response.length == 0) {
            ...
            if (this.repeated == false) {
              ...
              this.networkInitService.selectedAuthors =
  ...
  generateTablePublications(publications: any[]) {
    ...
    if (table instanceof HTMLElement) {
      ...
      publications.forEach(
        ({ title, DayOfPublication, AuthorName, PublicationType }) => {
  ...
  async waitNetworks() {
    while (
      !this.networkInitService.nameAuthors ||
      this.networkInitService.nameAuthors.length === 0
    ) {
